...
async function checkProgress() {
  ...
  for (const project of projects) {
    ...
    const [milestones] = await connection.execute(`
      SELECT id, title, status, dueDate, completedDate
      FROM milestones 
      WHERE projectId = ?
      ORDER BY id
    `, [project.id]);
    ...
    const expectedProgress = milestones.length > 0 ? Math.round((completed / milestones.length) * 100) : 0;
    ...
    milestones.forEach(m => {
      console.log(`   - ${m.title}: ${m.status} ${m.completedDate ? `(completado: ${m.completedDate})` : ''}`);
    });
  }

...
export async function generateProjectReport(
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    new Date(project.estimatedEndDate).toLocaleDateString("es"),
    rightCol + 30,
    rightY
  );
  rightY += 6;
  ...
  if (assignedEngineer) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      assignedEngineer.name || assignedEngineer.email || "N/A",
      rightCol + 30,
      rightY
    );
    rightY += 6;
  ...
  doc.text(
    `${completedMilestones} de ${milestones.length} hitos completados`,
    barX,
    barY + barHeight + 8
  );
  ...
  if (milestones.length > 0) {
    ...
    milestones.forEach((milestone, index) => {
      ...
      doc.setFont(
        "helvetica",
        milestone.status === "completed" ? "normal" : "bold"
      );
  ...
  const metrics = [
    ...
    { label: "Hitos Completados", value: completedMilestones.toString() },
    {
      ...
      value: (milestones.length - completedMilestones).toString(),
    },
    {
